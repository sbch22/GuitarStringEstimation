...
def sort_pitch_list(times, pitch_list):
    ...
    return times, np.array(pitch_list)
...
def frame_and_win(
    ...
    *,
    ...
    slices_return = xw[tuple(slices)]

    nb_frame_diff = slices_return.shape[-1]-n_frames

#    if len(x.shape)<=2:
    if nb_frame_diff<0:
        frames_to_cat = np.zeros(slices_return.shape[:-1]+(abs(nb_frame_diff),))
        slices_return = np.concatenate((slices_return,frames_to_cat),axis=-1)
    elif nb_frame_diff>0:
        slices_return = slices_return[...,:-nb_frame_diff]
    # else:
    #   if nb_frame_diff<0:
    #       frames_to_cat = np.zeros((frame_length,abs(nb_frame_diff)))
    #       slices_return = np.concatenate((slices_return,frames_to_cat),axis=-1)
    #   elif nb_frame_diff>0:
    #       slices_return = slices_return[...,:-nb_frame_diff]


    return slices_return*np.expand_dims(win,-1)
