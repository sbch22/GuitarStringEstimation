...
class matchNote:
    is_drum: bool
    program: int  # MIDI-Programmnummer (0-127)
    onset: float  # Startzeit in Sekunden
    onsetDiff: float  # Differenz zwischen vorhergesagtem und echtem Onset
    offset: float  # Endzeit in Sekunden
    pitch: int  # MIDI-Notennummer (0-127)
    velocity: int  # (0-1) wenn `ignore_velocity` True ist, sonst (0-127)
    contour: List[Tuple[float, float]]  # Liste von Tupeln mit (Zeit, Frequenz)
    string_index: int  # Index der Saite
...
class stringNote:
    ...
    pitch: int
    velocity: int
    noteBetas: Optional[List[float]] # Liste von Floats, die optional sein kann
    string_pred: Optional[int]  # Vorhergesagte Saite (kann None sein)
    likelihood_ratio: Optional[float]  # Wahrscheinlichkeit (kann None sein)
    stringGT: int  # Ground Truth Saite (immer vorhanden)
