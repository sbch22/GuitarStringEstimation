...
def build_audio_paths(base_name: str, data_dir: str) -> dict:
...
def preprocess_dataset(data_dir: str, save_dir: str):
    ...
    ann_files = glob.glob(os.path.join(data_dir, "annotation/*.jams"))
    assert len(ann_files) == 360

    train_list = load_csv_list("configs/trainSet.csv", add_solo=True)
    test_comp = load_csv_list("configs/testSet.csv")
    ...
    def handle_split(file_list, split, subset=None):
        for filename in file_list:
            filebase = filename.replace(".wav", "")
            track_id = filebase.split("_hex")[0]
            ann_path = os.path.join(data_dir, "annotation", f"{track_id}.jams")
